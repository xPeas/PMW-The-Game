...
public class CameraController : MonoBehaviour
{
    ...
    void LateUpdate()
    {
        ...
        void CheckPauseAndLockCursor()
        {
            /* Check to make sure the game isn't paused and lock the mouse cursor */
            if (Time.timeScale > 0.0f)
                Cursor.lockState = CursorLockMode.Locked;

            h += Input.GetAxis(StaticStrings.mouseX) * horizontalRotationSpeed * Time.deltaTime;
            v -= Input.GetAxis(StaticStrings.mouseY) * verticalRotationSpeed * Time.deltaTime;
